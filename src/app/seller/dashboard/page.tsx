...
"use client"  // ← Fix this line (remove the 's' and use double quotes)
...
import { 
  ...
  Search, Filter, Download, Upload, Menu, X
} from 'lucide-react'
...
function MobileHeader({ 
  ...
  return (
    <div className="lg:hidden bg-white border-b border-gray-200 p-4 sticky top-0 z-40"> {/* Fixed: removed space and added z-index */}
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div className="w-8 h-8 bg-gradient-to-r from-blue-600 to-purple-600 rounded-lg flex items-center justify-center"> {/* Fixed: bg-linear-to-r to bg-gradient-to-r */}
...
function SellerSidebar({ 
  ...
  return (
    <>
      ...
      {/* Sidebar - FIXED: Better mobile handling and z-index */}
      <motion.div
        initial={false}
        animate={{ 
          x: mobileOpen ? 0 : (typeof window !== 'undefined' && window.innerWidth < 1024 ? -320 : 0)
        }}
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4 lg:gap-6 mb-6 lg:mb-8">
      {statItems.map((item, index) => (
        ...
        >
          ...
          <h3 className="text-xl lg:text-2xl font-bold text-gray-900 mb-1">{item.value}</h3>
          <p className="text-gray-600 text-xs lg:text-sm">{item.label}</p>
        </motion.div>
      ))}
    </div>
  )
...
// =========================
// Products Table Component (RESPONSIVE)
// =========================
function ProductsTable({ 
  ...
  const getStatusBadge = (status: string | undefined) => {
    ...
    return (
      <span className={`inline-flex items-center gap-1 px-2 py-1 lg:px-3 lg:py-1 rounded-full text-xs font-medium border ${config.color}`}>
        {config.icon}
  ...
  const getStockBadge = (inStock: boolean | undefined) => {
    ...
    return (
      <span className={`inline-flex items-center px-2 py-1 lg:px-3 lg:py-1 rounded-full text-xs font-medium border ${
        safeInStock ? 'bg-green-100 text-green-800 border-green-200' : 'bg-red-100 text-red-800 border-red-200'
      }`}>
        <span className="hidden sm:inline">{safeInStock ? 'In Stock' : 'Out of Stock'}</span>
        <span className="sm:hidden">{safeInStock ? 'In' : 'Out'}</span>
      </span>
    )
  }
    ...
    <div className="bg-white rounded-xl lg:rounded-2xl shadow-sm border border-gray-200">
      ...
      <div className="p-4 lg:px-6 lg:py-4 border-b border-gray-200">
        <div className="flex flex-col space-y-4 lg:space-y-0 lg:flex-row lg:items-center lg:justify-between">
          <div>
            ...
            <p className="text-gray-600 text-sm hidden lg:block">Manage your product listings and inventory</p>
          </div>
      ...
      <div className="lg:hidden">
        {filteredProducts.map((product) => (
          <div key={product._id} className="p-4 border-b border-gray-200 hover:bg-gray-50 transition-colors">
            ...
            <div className="text-xs text-gray-500 mb-3">
              Updated: {product.updatedAt ? new Date(product.updatedAt).toLocaleDateString() : 'Unknown date'}
            </div>
...
export default function ProfessionalSellerDashboard() {
  ...
  return (
   <div className="flex min-h-screen bg-gray-50">
      ...
      {/* Main Content */}
        <div className="flex-1 lg:ml-0 min-w-0"> {/* Added min-w-0 to prevent overflow */}
        <MobileHeader 
          activeSection={activeSection} 
            ...
            >
              ...
              {activeSection === 'analytics' && (
                <div className="bg-white rounded-xl lg:rounded-2xl p-6 lg:p-8 shadow-sm border border-gray-200">
                  <div className="text-center py-8 lg:py-12">
                    <BarChart className="w-12 h-12 lg:w-16 lg:h-16 text-gray-400 mx-auto mb-4" />
                    <h3 className="text-lg lg:text-xl font-semibold text-gray-900 mb-2">Analytics Coming Soon</h3>
              ...
              {activeSection === 'add-product' && (
                <div className="bg-white rounded-xl lg:rounded-2xl p-6 lg:p-8 shadow-sm border border-gray-200">
                  <div className="text-center py-8 lg:py-12">
                    <Plus className="w-12 h-12 lg:w-16 lg:h-16 text-gray-400 mx-auto mb-4" />
                    <h3 className="text-lg lg:text-xl font-semibold text-gray-900 mb-2">Add New Product</h3>
                    <p className="text-gray-600 text-sm lg:text-base mb-4 lg:mb-6 max-w-md mx-auto">
                      Create new product listings that will be submitted for admin approval before going live.
                    </p>
                    <button
                      onClick={() => router.push('/seller/products/new')}
                      className="bg-blue-600 hover:bg-blue-700 text-white px-6 lg:px-8 py-3 rounded-lg font-semibold transition-colors text-sm lg:text-base"
                    >
                      Create Product
                    </button>
                  </div>
              ...
              {activeSection === 'settings' && (
                <div className="bg-white rounded-xl lg:rounded-2xl p-4 lg:p-6 xl:p-8 shadow-sm border border-gray-200 max-w-2xl">
                  <h3 className="text-lg lg:text-xl font-semibold text-gray-900 mb-4">Account Settings</h3>
                  {profileError && (
